...
class TestPositive:
    def test_step1(self, make_folder, clear_folder, make_files):
        result1 = ssh_checkout(data['host'], data['login'], data['passwd'],
                               'cd {}; 7z a {}/arx2 -t{}'.format(
                                   data['folder_in'], data['folder_out'],
                                   data['type']),
                               'Everything is Ok')
        result2 = ssh_checkout(data['host'], data['login'], data['passwd'],
                               'cd {}; ls'.format(data['folder_out']),
    ...
    def test1_hash(self, clear_folder, make_files):
        ...
        for i in make_files:
            ...
            hash = ssh_get(data['host'], data['login'], data['passwd'],
                           'cd {}; crc32 {}'.format(data['folder_in'],
                                                    i)).upper()
            res.append(
                ssh_checkout(data['host'], data['login'], data['passwd'],
                             'cd {}; 7z h {}'.format(data['folder_in'], i),
    ...
    def test_step2(self, clear_folder, make_files):
        ...
        for i in make_files:
            res.append(
                ssh_checkout(data['host'], data['login'], data['passwd'],
                             'ls {}'.format(data['folder_ext']), i))
        assert all(res), 'test2 Fail'

    def test_step3(self):
        assert ssh_checkout(data['host'], data['login'], data['passwd'],
                            'cd {}; 7z t arx2.{}'.format(data['folder_out'],
                                                         data['type']),
                            'Everything is Ok'), 'test3 Fail'

    def test_step4(self):
        assert ssh_checkout(data['host'], data['login'], data['passwd'],
                            'cd {}; 7z u {}/arx2.{}'.format(data['folder_in'],
                                                            data['folder_out'],
                                                            data['type']),
    ...
    def test_step5(self, clear_folder, make_files):
        ...
        for i in make_files:
            res.append(
                ssh_checkout(data['host'], data['login'], data['passwd'],
                             'cd {}; 7z l arx2.{}'.format(data['folder_out'],
                                                          data['type']),
                             i))
    ...
    def test_step6(self, clear_folder, make_files, make_subfolder):
        ...
        for i in make_files:
            res.append(
                ssh_checkout(data['host'], data['login'], data['passwd'],
                             'ls {}'.format(data['folder_ext2']), i))
        res.append(ssh_checkout(data['host'], data['login'], data['passwd'],
                                'ls {}'.format(data['folder_ext2']),
                                make_subfolder[0]))
        res.append(ssh_checkout(data['host'], data['login'], data['passwd'],
                                'ls {}/{}'.format(data['folder_ext2'],
                                                  make_subfolder[0]),
    ...
    def test_step7(self):
        assert ssh_checkout(data['host'], data['login'], data['passwd'],
                            'cd {}; 7z d arx2.{}'.format(data['folder_out'],
                                                         data['type']),
