...
def make_subfolder():
    ...
    if not ssh_checkout(data['host'], data['login'], data['passwd'],
                        'cd {}; mkdir {}'.format(data['folder_in'],
                                                 subfoldername), ''):
        return None, None
    if not ssh_checkout(data['host'], data['login'], data['passwd'],
...
def make_bad_arx():
    ssh_checkout(data['host'], data['login'], data['passwd'],
                 'cd {}; 7z a {}/bad_arx'.format(data['folder_in'],
                                                 data['folder_out']),
                 'Everything is Ok')
    ssh_checkout(data['host'], data['login'], data['passwd'],
...
def deploy():
    res = []
    upload_files(data['host'], data['login'], data['passwd'],
                 "/home/user/p7zip-full.deb", "/home/user1/p7zip-full.deb")
    res.append(ssh_checkout(data['host'], data['login'], data['passwd'],
                            "echo '111' | sudo -S dpkg -i /home/user1/p7zip-full.deb",
                            'Настраивается пакет'))
    res.append(ssh_checkout(data['host'], data['login'], data['passwd'],
                            "echo '111' | sudo -S dpkg -s p7zip-full",
